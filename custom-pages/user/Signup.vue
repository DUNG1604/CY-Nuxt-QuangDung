      ...
      <div class="flex flex-col bg-white shadow-md px-4 sm:px-6 md:px-8 lg:px-10 py-8 rounded-md w-full max-w-md">
        ...
        <div class="mt-10">
          <form @submit.prevent="handleSubmit">
            ...
            <div class="flex flex-col mb-6 relative">
              <label for="password" class="mb-1 text-xs sm:text-sm tracking-wide text-gray-600">ConfirmPassword:</label>
              <div class="relative">
                <div class="inline-flex items-center justify-center absolute left-0 top-0 h-full w-10 text-gray-400">
                  <span>
                    <svg class="h-6 w-6" fill="none" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
              ...
              <div v-if="!inputValue.checkConfirmPw" class="text-[12px] text-red-700 absolute top-[64px]">
                ConfirmPassword not
                match
              </div>
            </div>
        ...
        <div class="flex justify-center items-center mt-6">
          <div @click="navigateTo('/login')"
             class="cursor-pointer inline-flex items-center font-bold text-blue-500 hover:text-blue-700 text-xs text-center">
            <span>
              <svg class="h-6 w-6" fill="none" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                   viewBox="0 0 24 24" stroke="currentColor">
...
const clearInput = ()=>{
  ...
  inputValue.value.confirmPassword = "";
...
const handleSubmit = async () => {
  if (inputValue.value.password !== inputValue.value.confirmPassword) {
    inputValue.value.checkConfirmPw = false;
    inputValue.value.password = "";
    inputValue.value.confirmPassword = "";
    return;
  }
    ...
    const data = await userService.Signup(
        {
          ...
          name: inputValue.value.username,
